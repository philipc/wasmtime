...
fn get_base_type_name(type_entry: &write::ConvertUnitEntry<Reader<'_>>) -> Result<String, Error> {
    ...
    if let Some(die_ref) = type_entry.attr_value(gimli::DW_AT_type) {
        if let Some(ref die) = resolve_die_ref(type_entry.from_unit, &die_ref)? {
            if let Some(value) = die.attr_value(gimli::DW_AT_name) {
                return Ok(String::from(die.from_unit.attr_string(value)?.to_string()?));
            }
            match die.tag {
...
pub(crate) fn clone_unit<'a>(
    ...
    dbi_log!("Cloning CU {:?}", log_get_cu_summary(unit.from_unit));
    ...
    while let Some((entry_id, entry)) = unit.read_entry()? {
