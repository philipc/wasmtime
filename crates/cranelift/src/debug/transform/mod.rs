...
pub fn transform_dwarf(
    ...
    for (module, translation) in compilation.translations.iter() {
        ...
        let mut convert = out_dwarf.convert_with_filter(filter, Some(out_encoding))?;
        while let Some((mut unit, root_entry)) = convert.read_unit()? {
            if let Some(dwp) = dwarf_package.as_ref()
                && let Some(dwo_id) = unit.from_unit.dwo_id
                && let Ok(Some(split_dwarf)) = dwp.find_cu(dwo_id, unit.from_unit.dwarf)
            {
                let mut split_filter =
                    write::FilterUnitSection::new_split(&split_dwarf, unit.from_unit)?;
                build_dependencies(&mut split_filter, addr_tr)?;
                let mut convert_split = unit.convert_split_with_filter(split_filter)?;
                let (mut split_unit, root_entry) = convert_split.read_unit()?;
                clone_unit(
                    compilation,
                    module,
                    &mut split_unit,
