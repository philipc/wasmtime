...
pub(crate) fn clone_die_attributes<'a>(
    ...
    for attr in &entry.attrs {
        ...
        let out_attr_value = match attr_value {
            ...
            _ => match convert_unit
                .convert_attribute_value(unit, attr.name(), attr_value, &|_| None)
            {
                Ok(value) => value,
                Err(e) => {
                    ...
                    dbi_log!(
                        ...
                        entry.offset.to_unit_section_offset(&convert_unit.from_unit),
                        attr.name(),
                        attr_value,
                    );
                    continue;
